...
//   ];
  ...
  // var localStorage.setItem("names" ,JSON.stringify(text1));    //store arrey in string
  var storedNames = JSON.parse(localStorage.names)        //get in local storege
  ...
  form.addEventListener("submit", function (e) {
    ...
    // console.log(value);
    const random = Math.floor(Math.random() * storedNames.length);
    // console.log(value);
  
    if (isNaN(value) || value < 0 || value > 9) {
      result.innerHTML = `<p class="result">${storedNames[random]}</p>`;
      
    } else {
      let tempText = storedNames.slice(0, value);
      tempText = tempText
        .map(function (item) {
